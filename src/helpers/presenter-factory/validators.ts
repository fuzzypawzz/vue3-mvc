...
export function validateViewModelOverride(
  viewModelOverride: ViewModelOverride<PresenterConfig>,
  actualViewModel: Record<string, unknown>
) {
  const mockedViewModel = viewModelOverride(actualViewModel)

  const nonExistingProperties = Object.keys(mockedViewModel).filter(
    (key) => !(key in actualViewModel)
  )
